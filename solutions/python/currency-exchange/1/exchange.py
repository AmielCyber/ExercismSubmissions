...
def exchangeable_value(
    ...
    """
    rate_fee: float = exchange_rate + (exchange_rate * (spread / 100))
    budget_exchange_value: float = exchange_money(budget, rate_fee)
    number_of_bills: int = get_number_of_bills(budget_exchange_value, denomination)
