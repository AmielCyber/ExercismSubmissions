...
public class Anagram
{
    ...
    private bool IsAnagram(string otherWord)
    {
        ...
        Dictionary<char, int> _newWordCount = new(_wordCount);
        foreach (char ch in otherWord)
        {
            char lowerCaseCh = Char.ToLower(ch);
            if (!_newWordCount.ContainsKey(lowerCaseCh))
            {
                return false;
            }
            else
            {
                _newWordCount[lowerCaseCh] -= 1;
                if (_newWordCount[lowerCaseCh] == 0)
                {
                    _newWordCount.Remove(lowerCaseCh);
                }
            }
        }
        return _newWordCount.Count == 0;
    }
