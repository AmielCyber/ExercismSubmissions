...
public static class Dominoes
{
    public static bool CanChain(IEnumerable<(int, int)> dominoes)
    {
        ...
        HashSet<int> spareDominoesIndices = Enumerable.Range(1, dominoList.Count() - 1).ToHashSet();

        return CanChain(dominoList[0].rightVal, dominoList, spareDominoesIndices);
    }

    private static bool CanChain(
        int leftDominoRightVal,
        List<(int leftVal, int rightVal)> dominoList,
    ...
    private static bool CanChainDomino(
        int lDominoRightVal,
        (int leftVal, int rightVal) rightDomino
    ) => lDominoRightVal == rightDomino.leftVal || lDominoRightVal == rightDomino.rightVal;

    private static void ChainDomino(
        int lDominoRVal,
        int rDominoIndex,
        List<(int leftVal, int rightVal)> dList,
        HashSet<int> spareDominoes
    )
    {
    ...
    private static (int, int) GetRotatedDomino((int leftVal, int rightVal) domino) =>
        (domino.rightVal, domino.leftVal);

    private static void UnchainDomino(int rightDominoI, HashSet<int> spareDominoes) =>
        spareDominoes.Add(rightDominoI);
}
