...
public class SecurityPassMaker
{
    public string GetDisplayName(TeamSupport support)
    {
        if (support is Security)
            return GetSecurityDisplayName(support);
        if (support is Staff)
            return support.Title;
        return GetImportantMemberDisplayName();
    }

    private string GetSecurityDisplayName(TeamSupport security) => security.GetType() == typeof(Security) ? $"{security.Title} Priority Personnel" : security.Title;

    private string GetImportantMemberDisplayName() => "Too Important for a Security Pass";
}
