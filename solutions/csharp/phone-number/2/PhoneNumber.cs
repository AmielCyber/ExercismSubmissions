...
public class PhoneNumber
{
    ...
    private static void ValidatePhoneNumber(string phoneNumber)
    {
        ...
        int firstDigitInAreaCode = (int)Char.GetNumericValue(phoneNumber[0]);
        int firstDigitInLocalNum = (int)Char.GetNumericValue(phoneNumber[3]);

        if (N_DigitIsInvalid(firstDigitInAreaCode))
            throw new ArgumentException("First digit in area code must be greater than 1.");
        if (N_DigitIsInvalid(firstDigitInLocalNum))
            throw new ArgumentException("First digit in local phone number must be greater than 1.");
    }
