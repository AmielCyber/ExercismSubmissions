    ...
    private static string[] CustomSyntaxToForth(string[] instructions)
    {
        ...
        foreach (var instruction in instructions)
        {
            if (!UserDefinedSyntax.IsCustom(instruction))
            {
                ...
                {
                    builder.Append(UserDefinedSyntax.Contains(token)
...
{
    private static readonly Dictionary<string, ArithmeticOperation> Operations =
        new()
        {
            { "+", Add },
            { "-", Subtract },
    ...
    private static (int leftInteger, int rightInteger) GetIntegerValues(Stack<string> stack)
    {
        if (stack.Count < 2)
        {
            throw new InvalidOperationException("Can not perform operations without 2 operands in stack.");
        }
...
{
    private static readonly Dictionary<string, StackOperation> Operations =
        new()
        {
            { "dup", DuplicateTop },
            { "drop", DropTop },
    ...
    public static void PerformOperation(Stack<string> stack, string operation) => Operations[operation](stack);
...
public static class UserDefinedSyntax
{
    ...
    public static bool IsCustom(string instruction) => instruction.StartsWith(":") && instruction.EndsWith(";");
    ...
    private static void AssertOperationIsValid(string operation)
    {
        ...
        {
            throw new InvalidOperationException($"Operation '{operation}' is not a known forth operation.");
        }
    }

    private static bool IsOperation(string operation) => IntegerArithmetic.IsArithmetic(operation) ||
